function main() {
    if (!window.odoo?.info?.server_version?.includes("18.")) {
        console.log(`
            😿 Odoo Confirm does not support Odoo version ${window.odoo?.info?.server_version} (yet)
            Make a request at https://github.com/HarrisonHutton/odoo-confirm/issues
        `);
        return;
    }
    // BEGIN OWL-GENERATED CODE
    ...
    odoo.define('@odoo_confirm_module/core/common/composer_patch', ['@web/core/utils/patch', '@mail/core/common/composer', '@web/views/view_button/view_button', '@web/core/registry', '@mail/core/common/message_confirm_dialog', '@mail/utils/common/format', '@web/core/utils/hooks'], function (require) {
        'use strict';
        let __exports = {};
        ...
        patch(Composer.prototype, {
            ...
            async sendMessage() {
                if (this.props.type === "message" && this.props.mode === "extended") {
                    const text = this.props.composer.text;
